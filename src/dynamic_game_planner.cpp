...
void DynamicGamePlanner::run(TrafficParticipants& traffic_state) {
    ...
    setup();
    // Copy the initial state of each vehicle
    copyInitialState(traffic);
    ...
    final_constraint_violation = constraints_diagnostic(constraints, false);
    std::cout << "CV overall=" << (final_constraint_violation.any() ? "Yes" : "No")
              << " input_limits=" << (final_constraint_violation.input_limits ? "Yes" : "No")
              << " collision_avoidance=" << (final_constraint_violation.collision_avoidance ? "Yes" : "No")
              << " lane_boundaries=" << (final_constraint_violation.lane_boundaries ? "Yes" : "No")
              << std::endl;
...
void DynamicGamePlanner::integrate(SoA_X* X_, const SoA_U* U_)
{
    ...
    for (int i = 0; i < M; i++){
        ...
        for (int j = 0; j < param.N + 1; j++){
            ...
            typeInC ang = angle * param.ang_norm;
            typeInC sin_ang, cos_ang;
            TRIG_SINCOS(ang, sin_ang, cos_ang);
            ds_t0[x] = s_t0[v] * cos_ang;
            ds_t0[y] = s_t0[v] * sin_ang;
            ds_t0[v] = (-1.0/param.tau) * (param.tau_norm) * s_t0[v] + (param.k * param.k_norm) * u_t0[F];
            ds_t0[psi] = s_t0[v] * TRIG_TAN(u_t0[d] * param.ang_norm) * TRIG_COS(angle_1 * param.ang_norm)/ (param.length * param.length_norm * param.ang_norm);
...
void DynamicGamePlanner::quadratic_problem_solver(Eigen::MatrixXd & s_, const Eigen::MatrixXd & G_, const Eigen::MatrixXd & H_, double Delta)
{
    ...
    // The Cauchy step -Delta * G/||G|| is undefined at G = 0, where the
    // division evaluates as 0/0 and the non-finite value propagates to every
    // player through the coupled constraints. The Cauchy point is the zero
    // step there, since its numerator G^T G vanishes, so returning a zero step
    // is the definition rather than a substitute for it. A vehicle already at
    // its target speed reaches that state in exact arithmetic, at every
    // precision including fp64. The test is exact equality against zero, so
    // the branch is a no-op wherever the gradient is non-zero.
    if (normG == 0.0){
        s_ = Eigen::MatrixXd::Zero(G_.rows(), G_.cols());
        return;
    }
    ps = - Delta * (G_ /(normG));
    if ( GTHG <= 0.0){
...
/** Classifies c_v by the three families the reference study reports:
 *  collision avoidance, input limits and lane boundary conditions. */
ConstraintViolation DynamicGamePlanner::constraints_diagnostic(const double* constraints, bool print = false)
{
    ConstraintViolation violation;
    // The constraint vector for one vehicle is laid out in three blocks, in
    // this order. The bounds are half-open, so every index belongs to exactly
    // one family.
    const int input_end = 2 * param.nU * (param.N + 1);
    const int collision_end = input_end + (param.N + 1) * (M - 1);
    ...
    for (int i = 0; i < M; i++){
        for (int j = 0; j < nC_i; j++){
            if (constraints[nC_i * i +j] > 0){
                ...
                if (j < input_end) {
                    violation.input_limits = true;
                    family = "input";
                } else if (j < collision_end) {
                    violation.collision_avoidance = true;
                    family = "collision avoidance";
                } else {
                    violation.lane_boundaries = true;
                    family = "lane boundary";
                }
                if (print) {
                    std::cerr << "vehicle " << i << " violates " << family
                              << " constraint: " << constraints[nC_i * i + j] << "\n";
                }
            }
        }
        if (print == true){
...
void DynamicGamePlanner::trust_region_solver(SoA_U_Double& U_)
{
    ...
    // Convergence diagnostics. gradient_norm() returns the sum of squares and
    // the threshold is compared against that, so both are reported as squared
    // quantities alongside the norm.
    {
        const double gn = gradient_norm(gradient);
        const bool capped = (iter >= iter_lim);
        std::cout << "CONV iterations=" << iter
                  << " iter_limit=" << iter_lim
                  << " capped=" << (capped ? "yes" : "no")
                  << " termination=" << (capped ? "iteration_limit" : "gradient_threshold")
                  << " gradient_sq=" << gn
                  << " gradient_norm=" << std::sqrt(gn)
                  << " threshold_sq=" << threshold_gradient_norm
                  << " converged=" << (gn < threshold_gradient_norm ? "yes" : "no")
                  << std::endl;
    }
...
void DynamicGamePlanner::copyInitialState(const TrafficParticipants& traffic)
{   
    for (int i = 0; i < M; i++) {
        initial_state.x[i] = traffic[i].x;
        initial_state.y[i] = traffic[i].y;
        initial_state.v[i] = traffic[i].v;
        initial_state.psi[i] = traffic[i].psi;
        initial_state.v_target[i] = traffic[i].v_target;
    }
}
...
void DynamicGamePlanner::convertToWork(const SoA_U_Double* U_Double, const SoA_X_Double* X_Double, SoA_U* U_, SoA_X* X_)
{
...
void DynamicGamePlanner::convertFromWork(const SoA_X* X_, SoA_X_Double* X_Double)
{
    for(int i=0; i<NUM_State*MAX_OBSTACLES; i++)
    {
...
void DynamicGamePlanner::launch_integrate(SoA_X_Double* X_Double, const SoA_U_Double* U_Double)
{
    ...
    convertToWork(U_Double, X_Double, &U_, &X_);
    integrate(&X_, &U_);

    convertFromWork(&X_, X_Double);
