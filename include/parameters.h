...
struct Parameters
{
    ...
    // Normalization parameters, fixed by the reference study as quantities
    // intrinsic to the system: L = 64 m, V = 64 m/s, C = 4096. C is not an
    // independent choice: the nondimensional cost rate is w_hat = w*V*L/C,
    // so w_hat = 1 requires C = V*L.
