...
#include "trig.h"  // selects the trig path: native polynomial or fp64 conversion

#if USE_POSIT
#include <universal/number/posit/posit.hpp>
#elif REAL_BITS == 16 || REAL_BITS == 32
...
#elif REAL_BITS == 32
// binary32 as a library type rather than the builtin float. The planner's
// parameters are declared double, and C++ would promote a builtin float to
// double before multiplying. The library types round the double operand to the
// working format first, matching every other format in this study.
typedef sw::universal::single typeInC;
#else
...
/**
 * Constraint violation, split by the three families the reference study
 * reports. Its c_v is one classification covering all three: collision
 * avoidance, input limits, and lane boundary conditions. The individual flags
 * are kept so a failing run can be diagnosed by family.
 */
struct ConstraintViolation {
    bool input_limits = false;
    bool collision_avoidance = false;
    bool lane_boundaries = false;

    bool any() const {
        return input_limits || collision_avoidance || lane_boundaries;
    }
};
...
public:
    ...
    ConstraintViolation final_constraint_violation;
    ...
    ConstraintViolation constraints_diagnostic(const double* constraints, bool print); /** classifies and optionally prints violated constraints */
    ...
    void copyInitialState(const TrafficParticipants& traffic);
    ...
    void convertToWork(const SoA_U_Double* U_Double, const SoA_X_Double* X_Double, SoA_U* U_, SoA_X* X_);
    void convertFromWork(const SoA_X* X_, SoA_X_Double* X_Double);
